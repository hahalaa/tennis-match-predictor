...
# ==========================================
def add_features(df):
    print("⚙️  Engineering features... this may take a moment.")
    ...
    p1_surf_pct, p2_surf_pct, p1_h2h_gap = [], [], []

    for index, row in df.iterrows():
        p1, p2 = row['p1_name'], row['p2_name']
        surface, target = row['surface'], row['target']
        
        # --- LOOKUP PAST STATS ---
        def get_hist_stats(player, surf):
            if player in surface_history and surf in surface_history[player]:
                w, t = surface_history[player][surf]
                return w / t if t > 0 else 0.5
            return 0.5

        p1_surf_pct.append(get_hist_stats(p1, surface))
        p2_surf_pct.append(get_hist_stats(p2, surface))
        
        # H2H Lookup
        key = tuple(sorted([p1, p2]))
        if key in h2h_history:
            w1, w2 = h2h_history[key]
            # Calculate gap relative to P1
            diff = w1 - w2 if p1 == key[0] else w2 - w1
        else:
            diff = 0
        p1_h2h_gap.append(diff)

        # --- UPDATE HISTORY FOR FUTURE MATCHES ---
        # Update Surface Stats
        for player, won in [(p1, target == 1), (p2, target == 0)]:
            if player not in surface_history: surface_history[player] = {}
            if surface not in surface_history[player]: surface_history[player][surface] = [0, 0]
            
            surface_history[player][surface][1] += 1       # Add to total
            if won: surface_history[player][surface][0] += 1 # Add to wins

        # Update H2H Stats
        if key not in h2h_history: h2h_history[key] = [0, 0]
        winner_idx = 0 if (target == 1 and p1 == key[0]) or (target == 0 and p1 != key[0]) else 1
        h2h_history[key][winner_idx] += 1

    df['p1_surface_win_pct'] = p1_surf_pct
    df['p2_surface_win_pct'] = p2_surf_pct
    df['h2h_diff'] = p1_h2h_gap
